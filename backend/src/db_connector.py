...
class DbConnector:
    def __init__(self, db_user=db_user, db_passwd=db_passwd, db_host=db_host, db_name=db_name, use_sqlite=False, sqlite_db_path='output/local_database.db'):
        self.use_sqlite = use_sqlite
        if use_sqlite:
            print(f"Connecting to SQLite database at {sqlite_db_path}")
            # SQLite connection string
            self.url = f"sqlite:///{sqlite_db_path}"
        ...
        if df.shape[0] > 0:
            if df.shape[0] > batch_size:
                ...
                while batch_start < total_length:
                    ...
                    upsert(con=self.engine, df=df.iloc[batch_start:batch_end],
                           table_name=table_name, if_row_exists=if_exists, create_table=False)
                    print("Batch " + str(batch_end))
                    batch_start = batch_end
            else:
                upsert(con=self.engine, df=df, table_name=table_name,
...
# ------------------------- additional db functions --------------------------------
    ...
    def get_last_price_eth(self, origin_key: str, granularity: str = 'daily'):
        ...
        try:
            ...
            print(
                f"Error retrieving the latest price in ETH for {origin_key}.")
            print(e)
            return None
    ...
    def get_last_price_usd(self, origin_key: str, granularity: str = 'daily'):
        ...
        try:
            ...
            print(
                f"Error retrieving the latest price in USD for {origin_key}.")
            print(e)
            return None
    ...
    def get_main_config_dict(self):
        ...
        for column in ['api', 'aliases', 'metadata', 'socials', 'runs', 'backfiller', 'cross_check', 'circulating_supply']:
            df = pd.concat([df.drop([column], axis=1), df[column].apply(
                pd.Series).add_prefix(column + '_')], axis=1)

        df = df.where(pd.notnull(df), None)
        df['backfiller_batch_size'] = df['backfiller_batch_size'].fillna(
            0). astype(int)
    ...
    def get_economics_in_eth(self, days, exclude_chains, origin_keys=None, incl_profit=True):
        if exclude_chains is None or len(exclude_chains) == 0:
            ...
            exclude_string = "AND tkd.origin_key not in ('" + "', '".join(
                exclude_chains) + "')"

        if origin_keys is None or len(origin_keys) == 0:
            ok_string = ''
        else:
            ok_string = "AND tkd.origin_key in ('" + \
                "', '".join(origin_keys) + "')"
    ...
    # also make sure to add new metrics in adapter_sql
    def get_values_in_eth(self, metric_keys, days, origin_keys=None):
    ...
    # also make sure to add new metrics in adapter_sql
    def get_values_in_usd(self, metric_keys, days, origin_keys=None):
        mk_string = "'" + "', '".join(metric_keys) + "'"
    ...
    def aggregate_unique_addresses(self, chain: str, days: int, days_end: int = None):
        ...
        print(
            f"Unique addresses for {chain} and {days} days aggregated and loaded into fact_active_addresses.")
    ...
    def aggregate_unique_addresses_hll(self, chain: str, days: int):
        ...
        with self.engine.connect() as connection:
            with connection.begin():
                connection.execute(text(exec_string))
        print(
            f"HLL hashes for {chain} and {days} days loaded into fact_active_addresses_hll.")
    ...
    def aggregate_addresses_first_seen_global(self, days: int):
        ...
        with self.engine.connect() as connection:
            with connection.begin():
                connection.execute(text(exec_string))
        print(
            f"Unique addresses with first seen for {days} days loaded into fact_unique_addresses.")
 ...
 # This function is used to get the inscriptions per chain. The data will be loaded into fact_sub_category_level table
        ...
        for index, row in df.iterrows():
            if column_type == 'str':
                ...
                raise NotImplementedError(
                    "Only string type is supported so far")
    ...
    def get_block_by_date(self, table_name: str, date: datetime):
        ...
        if 'block_timestamp' not in columns:
            raise ValueError(
                f"Table {table_name} does not have a 'block_timestamp' column.")
        ...
        if block_number is None:
            raise ValueError(
                f"No block found for the date {date_str} in table {table_name}.")
